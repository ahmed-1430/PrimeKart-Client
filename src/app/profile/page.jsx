...
export default function ProfilePage() {
        ...
        async function loadData() {
            try {
                ...
                setStats({
                    ...
                    spent: orders.reduce((acc, o) => acc + o.total, 0),
                });
    ...
    const handleSave = async () => {
        try {
            await axios.put(`http://localhost:3000/api/users/${user.email}`, profile);
    ...
    return (
        <div className="max-w-6xl mx-auto p-6 mt-10 space-y-12">
            ...
            {/* PROFILE HEADER CARD */}
            <div className="p-10 rounded-3xl bg-linear-to-br from-purple-600/20 to-blue-500/20 backdrop-blur-xl 
                            shadow-[0_8px_30px_rgba(0,0,0,0.08)] border border-white/20 relative overflow-hidden">
                ...
                <div className="relative flex flex-col md:flex-row items-center gap-8 z-10">
                    ...
                    {/* User Avatar */}
                    <div className="w-32 h-32 rounded-full bg-linear-to-br from-purple-600 to-blue-500 flex items-center justify-center 
                                    text-4xl font-bold text-white shadow-xl border border-white/20">
            ...
            <div className="grid md:grid-cols-3 gap-6">
                {[
                    ...
                    { label: "Total Spent", value: `$${stats.spent}`, color: "green" },
                ].map((box, idx) => (
                    <div
                        key={idx}
                        className="p-6 rounded-2xl bg-white shadow-lg border border-zinc-200 hover:shadow-xl transition-all duration-300 
                                    hover:-translate-y-1"
                    >
                        <h3 className="text-gray-500">{box.label}</h3>
                        <p className={`text-4xl font-bold mt-3 text-${box.color}-600`}>{box.value}</p>
                    </div>
                ))}
            </div>
            ...
            <div className="p-10 bg-white shadow-2xl rounded-3xl border border-zinc-100">
                ...
                <button
                    onClick={handleSave}
                    className="mt-6 px-8 py-3 bg-linear-to-r from-purple-600 to-blue-600 text-white rounded-xl 
                              text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-200"
                >
                    Save Changes
                </button>
            </div>
            ...
            <div className="p-10 bg-white rounded-3xl shadow-2xl border border-zinc-200">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <Link href="/orders" className="text-purple-600 font-medium hover:underline">
                        View All
                    </Link>
                </div>
                    ...
                    <div className="space-y-4">
                        {recentOrders.map((o) => (
                            ...
                            >
                                ...
                                <p className="font-bold text-purple-600 mt-1">${o.total}</p>
                            </div>
                        ))}
                    </div>
                )}
